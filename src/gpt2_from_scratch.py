...
def train_gpt2(model, dataloader, optimizer, device, num_epochs=5, save_dir=None, save_every=None):
    ...
    Args:
        ...
        save_every (int, optional): チェックポイント保存間隔（エポック単位）。
...
def parse_arguments():
    ...
    parser.add_argument('--num_epochs', type=int, default=3, help='エポック数 (default: 3)')
...
def main():
    ...
    dataset = TextDataset(sample_text, tokenizer, max_length=128)
    ...
    final_model_path = train_gpt2(
        ...
        num_epochs=args.num_epochs, save_dir=args.save_dir, save_every=1
    )
